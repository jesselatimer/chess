...
class Display
  ...
  attr_reader :board, :selection_mode
  ...
  #REFACTOR
  def execute_command(input) ## REFACTOR THIS
    if input == :quit
      abort
    elsif input == :select
      if selection_mode && valid_move?(cursor_pos, secondary_cursor) &&
        ...
        unless board[cursor_pos].empty?(cursor_pos)
          @selection_mode = !@selection_mode
          @secondary_cursor = @cursor_pos if selection_mode
          @cursor_pos = @secondary_cursor unless selection_mode
        end
      else
        @secondary_cursor = @cursor_pos if selection_mode
        @selection_mode = false
